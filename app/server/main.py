...
async def infer(video_file: fastapi.UploadFile):
    ...
    start_time, end_time = await video.get_player_times(original_path, boxes_list)
    player_ids = set(start_time.keys())
    player_ids_header = ",".join(str(pid) for pid in sorted(player_ids))
    player_times_header = ",".join(f"{start_time[pid]}-{end_time[pid]}" for pid in sorted(player_ids))

    await video.draw_bboxes(original_path, ANNOTATED_PATH, boxes_list, {pid: video.PlayerParams() for pid in player_ids})
    ...
    return fastapi.responses.FileResponse(
        path=ZIP_PATH, headers={"player_ids": player_ids_header, "player_times": player_times_header}
    )
...
async def make_video(player_params: dict[int, video.PlayerParams]):
    ...
    await video.draw_bboxes(original_path, ANNOTATED_PATH, boxes_list, player_params)
...
async def make_focused_video(player_id: int):
    ...
    await video.crop_to_player(original_path, ANNOTATED_PATH, boxes_list, player_id)
    return ANNOTATED_PATH
